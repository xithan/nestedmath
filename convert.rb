...
def close_file(file)
  #file.puts "\\begingroup\\renewcommand{\\chapter}[2]{}\\endgroup"
  file.puts '\end{document}'
  file.close
end

def cleanup name
    #system( "cd #{$outdir}; find . -type f -name '#{name}.*' -not -name '*.pdf' -not -name '*.tex' | xargs rm")
end
...
def run_latex name, format=:latex, workdir = $workdir
  ...
  when :pdflatex 
    ...
    puts "cd #{workdir}; pdfetex -output-format pdf  -file-line-error -synctex=0 -fmt '#{format}'  './#{name}.tex' "
    return system("cd #{workdir}; pdfetex -output-format pdf  -file-line-error -synctex=0 -fmt '#{format}'  './#{name}.tex' ")
  end
end
...
def typeset(file = $outfile, format = :mathview)
 ...
 # run_latex  file, format.to_s
  #system("cd #{$outdir} ; pdfetex -output-format pdf -interaction batchmode -synctex=0 -fmt #{format.to_s}  './#{file}.tex'")
  if format == :mathview
    run_bibtex file
    #run_latex  file, format.to_s
  #  run_bibtex file
  ...
  cleanup file unless $no_cleanup
  #system("cd #{$outdir} && pdflatex #{file} -halt-on-error")
end
...
def typeset_preamble workdir=$workdir
  ...
  # Die beiden nachfolgenden auskommentireten funktionieren auf jeden Fall.
  # run_initex ist noch ungetest
  #system("perl '#{$indir}generate_format.pl' '#{$outdir}mathview.tex' --fmt=mathview")
  #system("perl '#{$indir}generate_format.pl' '#{$outdir}defview.tex' --fmt=defview")
  run_initex "mathview"
  run_initex "defview"
#  system("cd #{$indir} ; perl generate_format.pl #{$outdir}mathview.tex --fmt=#{$outdir}mathview")
  
#  Dir.chdir("/Users/drkuehl/Dropbox/Doktorarbeit/TsoReordered/Thesis/pdfjs/thesis")
#  system("pdfetex -output-format pdf -ini -interaction batchmode -synctex=0 -recorder -file-line-error &pdflatex './defview.ini'")
  
  # system("cd #{$outdir} ; pdfetex -output-format pdf -ini -interaction batchmode -synctex=0 -recorder -file-line-error &pdflatex './mathview.ini'")
end
...
unless $no_cleanup
  ...
  # ['log','aux','bbl','dvi','out','tex','maf'].each do |t|
  #   system("cd #{$workdir}; find . -iname '*.#{t}' -exec rm {} \\;")
  # end
end
